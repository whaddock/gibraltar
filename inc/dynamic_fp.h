...
struct dynamic_fp {
	...
	int (*gib_free)(void *buffers, struct gib_context_t *c);
        int (*gib_generate)(void *buffers, int buf_size, int stream,
			    struct gib_context_t *c);
        int (*gib_generate2)(void *buffers, unsigned int buf_size,
			    struct gib_context_t *c);
	int (*gib_generate_nc)(void *buffers, int buf_size, int work_size,
			       struct gib_context_t *c);
	int (*gib_recover)(void *buffers, size_t buf_size, unsigned int *buf_ids,
			   int recover_last, int stream, struct gib_context_t *c);
	int (*gib_recover2)(void *buffers, unsigned int buf_size, unsigned int *buf_ids,
