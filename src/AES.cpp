...
{
	if(this->isCopy) {
	  std::cout <<"Is not posible to modify a shallow copied AES object."
		    << std::endl;
	} else {
		if (this->key == NULL) {
			if(bits != 128 && bits != 192 && bits != 256) {
			  std::cout << "AES only support a key-length of 128, 192, or 256 bits."
			  << std::endl;
			}
			this->keyBits = bits;
			int bytes = bits/8;
			this->key = (unsigned char*) malloc(bytes);
			std::memcpy(this->key,key,bytes);
		}
	}
    return 0;
}
...
int paracrypt::AES::getKeyBits(int rounds)
{
	...
	switch(rounds) {
		...
		case 14:
			...
			break;
		default:
		  std::cout << "Invalid number of rounds. AES only support 10, 12, or 14 rounds."
	  << std::endl;
	}
	return key_bits;
}
